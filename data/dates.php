...
$query_datesRS = "SELECT * FROM schedule_title ORDER BY schedule_title_id ASC";
$datesRS = mysql_query($query_datesRS, $abtaFrontConn) or die(mysql_error());
$row_datesRS = mysql_fetch_assoc($datesRS);
$totalRows_datesRS = mysql_num_rows($datesRS);
...
if (isset($_GET['view_xml'])) {
	...
	echo "<recordset total=\"".$totalRows_datesRS."\">";
	if($totalRows_datesRS > 0){
		$Start_Record = 0;
		$Num_Records = $totalRows_datesRS;
		$Current_Record = 0;
		...
		$totalColumns=mysql_num_fields($datesRS);
		do{
			if($Current_Record>=$Start_Record){
				echo "<record>";
				for ($x=0; $x<$totalColumns; $x++) {
					$field=mysql_field_name($datesRS, $x);
					echo "<".$field."><"."![CD"."ATA[".$row_datesRS[$field]."]"."]></".$field.">";
				}
				echo "</record>";
			}
			$Current_Record=$Current_Record+1;
			if($Current_Record-$Start_Record==$Num_Records){break;}
		}while ($row_datesRS = mysql_fetch_assoc($datesRS)); 
	}
	echo "</recordset>";
	die(); 
}
