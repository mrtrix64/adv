...
//LOCAL
$db = mysql_connect("localhost","db99712_anne","treestump2010");
    if (!$db) {
        die('Could not connect to db: ' . mysql_error());
    }
    ...
    //Select the Database
    mysql_select_db("db99712_advantage",$db);
    
    //Replace * in the query with the column names.
    $result = mysql_query("SELECT * FROM delegates ORDER BY Organization ASC", $db);  
...
header("Content-Type: application/xml");
 echo "<recordset>"."\n";
    while ($row = mysql_fetch_array($result, MYSQL_ASSOC)) {
		...
		echo "<First_Name><![CDATA[".($row['First_Name'])."]]></First_Name>"."\n";
		echo "<Last_Name><![CDATA[".($row['Last_Name'])."]]></Last_Name>"."\n";
		echo "<Organization><![CDATA[".($row['Organization'])."]]></Organization>"."\n";
